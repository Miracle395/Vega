...
const [suiPriceObjId, dbusdcPriceObjId] = await pythClient.updatePriceFeeds(tx, updateData, feedIds);
return { suiPriceObj: tx.object(suiPriceObjId), dbusdcPriceObj: tx.object(dbusdcPriceObjId) };
}
...
window.ensureMarginManager = async function() {
  ...
  const cacheKey = 'vega_marginManagerId_' + addr;
  const cached = localStorage.getItem(cacheKey);
  if (cached) {
    window._marginManagerId = cached;
    return cached;
  }
...
tx.setGasBudget(500_000_000);
  ...
  const { suiPriceObj, dbusdcPriceObj } = await _freshenPythPrices(tx, suiClient);
