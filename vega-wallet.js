...
/* ── SUI NETWORK CONFIG ─────────────────────────────────────── */
const SUI_CONFIG = {
  // Match this to WALRUS_CONFIG.network in walrus.js
...
async function _fetchWalletBalances(addr) {
  try {
    const cfg = _suiCfg();
    // Use Tatum RPC from trade-executor config if available
    const execNet = (typeof EXECUTOR_CONFIG !== 'undefined') ? EXECUTOR_CONFIG.network.toUpperCase() : null;
    const rpcUrl  = execNet ? EXECUTOR_CONFIG[execNet].tatumRpc : cfg.rpcUrl;
    const apiKey  = execNet ? EXECUTOR_CONFIG[execNet].tatumKey : null;
    ...
    console.log('[Wallet] Balances - SUI: ' + suiBalance.toFixed(4) + ', USDC: ' + usdcBalance.toFixed(2));
  } catch (err) {
