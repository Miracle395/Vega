...
if (isBaseIn) {
  ...
  const quoteType = dbMod.testnetCoins?.DBUSDC?.type || coins?.DBUSDC?.type;
const baseType  = '0x2::sui::SUI';
const [zeroQuote] = tx.moveCall({ target: '0x2::coin::zero', typeArguments: [quoteType] });
const [zeroBase]  = tx.moveCall({ target: '0x2::coin::zero', typeArguments: [baseType]  });

  const _qty = isBaseIn
    ? BigInt(Math.round(amountIn * 1_000_000_000))
    : BigInt(Math.round(amountIn * 1_000_000));

  const [baseOut, quoteOut, deepOut] = isBaseIn
 ...
 tx.transferObjects([baseOut, quoteOut], walletAddr);
tx.moveCall({ target: '0x2::coin::destroy_zero', typeArguments: [deepType], arguments: [deepOut] });
