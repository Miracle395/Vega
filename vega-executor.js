...
const { Transaction, coinWithBalance } = await import('https://esm.sh/@mysten/sui@1.21.1/transactions?bundle=false');

    console.log('[Executor] DeepBook SDK modules loaded');
    return { DeepBookClient, Transaction, coinWithBalance, dbMod };
  })();
...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  /* ── Load SDK ─────────────────────────────────────────────── */
  let DeepBookClient, Transaction, dbMod, coinWithBalance;
  try {
    ({ DeepBookClient, Transaction, dbMod, coinWithBalance } = await Promise.race([
 ...
 // Build the input coin via CoinWithBalance — splits exactly the amount
  ...
  let inputCoin;
  if (isBaseIn) {
    inputCoin = coinWithBalance({ balance: BigInt(Math.round(amountIn * 1e9)) }); // SUI, 9 decimals
  } else {
    ...
    inputCoin = coinWithBalance({ type: dbusdcInfo.type, balance: BigInt(Math.round(amountIn * scalar)) });
  }
