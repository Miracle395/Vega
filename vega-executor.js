...
async function _loadDeepBookModules() {
  ...
  import('https://esm.sh/@mysten/sui'),
   
  import('https://esm.sh/@mysten/sui/transactions'),
]);
const DeepBookClient = dbMod.DeepBookClient;
const SuiClient = suiClientMod.SuiClient || suiClientMod.default?.SuiClient;
console.log('[Executor] SuiClient:', typeof SuiClient, 'suiMod top keys:', Object.keys(suiClientMod).slice(0, 10));
...
const packageIds = network === 'testnet'
  ? dbMod.testnetPackageIds
  : dbMod.mainnetPackageIds;
...
tx.setSenderIfNotSet(walletAddr);
try {
