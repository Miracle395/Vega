...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  const cfg2 = _execCfg();
  const coinType = isBaseIn ? '0x2::sui::SUI' : dbMod.testnetCoins?.DBUSDC?.type || coins?.DBUSDC?.type;
  ...
  if (!coinObj) throw new Error(`[Executor] No ${isBaseIn ? 'SUI' : 'DBUSDC'} coin found in wallet`);
