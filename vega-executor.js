...
const EXECUTOR_CONFIG = {
  network: 'MAINNET',        // 'TESTNET' | 'MAINNET'  — must match walrus.js + sui-wallet.js
...
async function _loadDeepBookModules() {
  ...
  _dbModulesPromise = (async () => {
    const [{ DeepBookClient }, { SuiClient, getFullnodeUrl }, { Transaction }] = await Promise.all([
      import('https://esm.sh/@mysten/deepbook-v3'),
      import('https://esm.sh/@mysten/sui/client'),
      import('https://esm.sh/@mysten/sui/transactions'),
    ]);
    console.log('[Executor] DeepBook SDK modules loaded');
    return { DeepBookClient, SuiClient, getFullnodeUrl, Transaction };
  })();
...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  /* ── Load DeepBook SDK ────────────────────────────────────── */
  let DeepBookClient, SuiClient, getFullnodeUrl, Transaction;
  try {
    ({ DeepBookClient, SuiClient, getFullnodeUrl, Transaction } = await Promise.race([
  ...
  try {
    if (isBaseIn) {
      outputs = tx.add(dbClient.deepBook.swapExactBaseForQuote({
        poolKey,
        amount: amountIn,
        deepAmount: 0,   // pay fee in input token instead of DEEP
        minOut,
      }));
    } else {
      outputs = tx.add(dbClient.deepBook.swapExactQuoteForBase({
        poolKey,
        amount: amountIn,
        deepAmount: 0,
        minOut,
    ...
    const [baseOut, quoteOut, deepOut] = outputs;
...
window.fetchOrderbook = async function(sym) {
  return null; // Handled by _fetchBirdeyeOB in gambit.js
};
console.log('[Executor] trade-executor.js loaded — Aftermath Finance router ready');
