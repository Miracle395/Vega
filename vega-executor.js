...
async function _loadDeepBookModules() {
  ...
  _dbModulesPromise = (async () => {
    ...
    const { Transaction } = await import('https://esm.sh/@mysten/sui@1.21.1/transactions?bundle=false');

    console.log('[Executor] DeepBook SDK modules loaded');
    return { DeepBookClient, Transaction, dbMod };
  })();
...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  /* ── Load SDK ─────────────────────────────────────────────── */
  let DeepBookClient, Transaction, dbMod;
  try {
    ({ DeepBookClient, Transaction, dbMod } = await Promise.race([
  ...
  const suiClient = _makeSuiRpcClient(cfg.rpcUrl);
  ...
  if (typeof tx.setClient === 'function') tx.setClient(suiClient);
  ...
  // Fetch coins manually to avoid CoinWithBalance intent requiring a full SuiClient
  const cfg2 = _execCfg();
  const coinType = isBaseIn ? '0x2::sui::SUI' : dbMod.testnetCoins?.DUSDC?.type || coins?.DUSDC?.type;
  const coinData = await fetch(cfg2.rpcUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ jsonrpc:'2.0', id:1, method:'suix_getCoins', params:[walletAddr, coinType, null, 1] }),
  }).then(r => r.json());

  const coinObj = coinData.result?.data?.[0];
  if (!coinObj) throw new Error(`[Executor] No ${isBaseIn ? 'SUI' : 'DUSDC'} coin found in wallet`);

  const inputCoin = tx.object(coinObj.coinObjectId);

  const [baseOut, quoteOut, deepOut] = dbClient.deepBook.swapExactQuantity({
    poolKey,
    isBaseToCoin: isBaseIn,
    amount:       amountIn,
    deepAmount:   0,
    minOut,
...
function _resolvePoolAndSide(from, to) {
  ...
  if (F === 'SUI'  && T === 'USDC') return { poolKey: 'SUI_DUSDC', isBaseIn: true  };
  if (F === 'USDC' && T === 'SUI')  return { poolKey: 'SUI_DUSDC', isBaseIn: false };
