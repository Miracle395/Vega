...
async function _loadDeepBookModules() {
  ...
  _dbModulesPromise = (async () => {
    const [{ deepbook: DeepBookClient }, { SuiGrpcClient }, { Transaction }] = await Promise.all([
...
const dbClient = new SuiGrpcClient({ network, baseUrl })
  .$extend(DeepBookClient({ address: walletAddr }));
