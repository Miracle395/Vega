...
const { Transaction, coinWithBalance, SuiClient } = await import('https://esm.sh/@mysten/sui@1.21.1?bundle');

    console.log('[Executor] DeepBook SDK modules loaded');
    return { DeepBookClient, Transaction, coinWithBalance, SuiClient, dbMod };
  })();
...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  /* ── Load SDK ─────────────────────────────────────────────── */
  let DeepBookClient, Transaction, dbMod, coinWithBalance, SuiClient;
  try {
   ({ DeepBookClient, Transaction, dbMod, coinWithBalance, SuiClient } = await Promise.race([
