...
if (isBaseIn) {
  const coinData = await fetch(cfg2.rpcUrl, {
    ...
    body: JSON.stringify({
      ...
      params: [walletAddr, '0x2::sui::SUI', null, 1]
    }),
  }).then(r => r.json());

  coinObj = coinData.result?.data?.[0];
} else {
  const dbusdcType =
    dbMod.testnetCoins?.DBUSDC?.type ||
    coins?.DBUSDC?.type;

  const dusdcType =
    dbMod.testnetCoins?.DUSDC?.type ||
    coins?.DUSDC?.type;

  let coinData = await fetch(cfg2.rpcUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      jsonrpc: '2.0',
      id: 1,
      method: 'suix_getCoins',
      params: [walletAddr, dbusdcType, null, 1]
    }),
  }).then(r => r.json());

  coinObj = coinData.result?.data?.[0];

  // Fallback to DUSDC if no DBUSDC found
  if (!coinObj && dusdcType) {
    coinData = await fetch(cfg2.rpcUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: 1,
        method: 'suix_getCoins',
        params: [walletAddr, dusdcType, null, 1]
      }),
    }).then(r => r.json());
...
if (!coinObj) {
  throw new Error(
    `[Executor] No ${isBaseIn ? 'SUI' : 'DBUSDC or DUSDC'} coin found in wallet`
  );
}
