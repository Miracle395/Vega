...
async function _loadDeepBookModules() {
  ...
  _dbModulesPromise = (async () => {
    const [dbMod, { SuiGrpcClient }, { Transaction }] = await Promise.all([
...
const suiClient = new SuiClient({ network, baseUrl: rpcUrl });
...
try {
  ...
  try {
    ...
    const builtTx = await tx.build({ client: suiClient.core });
