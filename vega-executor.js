...
async function _loadDeepBookModules() {
  ...
  _dbModulesPromise = (async () => {
    const [{ deepbook }, { SuiGrpcClient }, { Transaction }] = await Promise.all([
  import('https://esm.sh/@mysten/deepbook-v3'),
  import('https://esm.sh/@mysten/sui/grpc'),
  import('https://esm.sh/@mysten/sui/transactions'),
]);
const DeepBookClient = deepbook;
console.log('[Executor] DeepBook SDK modules loaded');
return { DeepBookClient, SuiGrpcClient, Transaction };
  })();
...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  /* ── Load DeepBook SDK ────────────────────────────────────── */
  let DeepBookClient, SuiGrpcClient, Transaction;
try {
  ({ DeepBookClient, SuiGrpcClient, Transaction } = await Promise.race([
...
const dbClient = suiClient.$extend(DeepBookClient({ address: walletAddr }));
