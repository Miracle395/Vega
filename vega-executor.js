...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
 ...
 // Use SuiClient (JSON-RPC) not SuiGrpcClient — needed for tx.build()
  const suiMod = await import('https://esm.sh/@mysten/sui/client');
  const SuiJsonClient = suiMod.SuiClient || suiMod.default?.SuiClient || suiMod.default;
  if (!SuiJsonClient || typeof SuiJsonClient !== 'function') {
    throw new Error('[Executor] Could not resolve SuiClient constructor from @mysten/sui/client');
  }
