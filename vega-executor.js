...
window.executeSpotTrade = async function({ from, to, amountIn, slippage = 0.5 }) {
  ...
  if (_balanceManagerId) {
    // Inject cached manager into the SDK's internal map
    if (dbClient.balanceManager?.balanceManagers) {
    ...
    // First ever run — create the manager on-chain
    dbClient.balanceManager.createAndShareBalanceManager()(tx);
    managerKey = null; // swap skipped this round, ID extracted post-exec
  }
  ...
  let txDigest;
  try {
    // Pass Transaction object directly to wallet — modern wallets handle build internally
  ...
  /* ── Post-exec: extract BalanceManager ID if first run ───── */
  if (!managerKey) {
    try {
      const info = await fetch(cfg.rpcUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          jsonrpc: '2.0', id: 1,
          method: 'sui_getTransactionBlock',
          params: [txDigest, { showObjectChanges: true }],
        }),
      }).then(r => r.json());

      const created = info.result?.objectChanges?.find(
        c => c.type === 'created' && c.objectType?.includes('BalanceManager')
      );
      if (created?.objectId) {
        _balanceManagerId = created.objectId;
        try { localStorage.setItem('vega_bm_id', _balanceManagerId); } catch(_) {}
        console.log('[Executor] BalanceManager created:', _balanceManagerId);
      }
    } catch (e) {
      console.warn('[Executor] Could not extract BalanceManager ID:', e);
    }
    ...
    return {
      ...
      note: 'BalanceManager initialized. Submit your trade again to execute the swap.',
    };
  }
