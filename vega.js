...
const MARKETS = [
  { sym:'SUI/USDC',  name:'Sui',         cat:'spot crypto', price:null, chg:null, vol:null, color:'#4DA2FF', tv:'BINANCE:SUIUSDT',  spot:true,
    coinIn:  '0x65b0553a591d7b13376e03a408e112c706dc0909a79080c810b93b06f922c458::usdc::USDC',
    coinOut: '0x2::sui::SUI',
  },
...
// Aftermath swap event type — real on-chain trades
...
// Pull real swap events from Aftermath on-chain
...
async function _loadDefaultRoute() {
  try {
    const afSdk = await Promise.race([
      _loadAftermathSdk?.() ?? Promise.reject(),
      new Promise((_, r) => setTimeout(() => r(new Error('timeout')), 12000)),
    ]);
    const router = afSdk.Router();
    const route  = await router.getCompleteTradeRouteGivenAmountIn({
      coinInType:  '0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7::usdc::USDC',
      coinOutType: '0x2::sui::SUI',
      coinInAmount: BigInt(10_000_000),
    });
    if (!route) throw new Error('empty route');
    const hops   = (route.routes || []).flatMap(r => r.paths || []);
    const venues = hops
      .map(p => p.pool?.protocol || p.pool?.name || 'Aftermath')
      .filter((v, i, arr) => arr.indexOf(v) === i)
      .map(v => ({ venue: v, type: 'DEX' }));
    const score = Math.min(95, 70 + venues.length * 8);
    window.onRouteTick({
      inToken:  'USDC',
      outToken: STATE.marketSym.split('/')[0],
      hops:     venues.length ? venues : [{ venue: 'Aftermath', type: 'DEX' }],
      score,
    });
    const routeEl = document.getElementById('strip-stat-route');
    if (routeEl) {
      setRouteScoreBadge(routeEl, score);
    }
  } catch (e) {
    console.warn('[Route] Aftermath SDK unavailable, using fallback:', e.message);
    window.onRouteTick({
      inToken:  'USDC',
      outToken: STATE.marketSym.split('/')[0],
      hops:     [{ venue: 'Aftermath', type: 'DEX' }],
      score:    78,
    });
    const routeEl = document.getElementById('strip-stat-route');
   if (routeEl) setRouteScoreBadge(routeEl, 78);
  }
}
...
window.coinIconHTML = function(icon, init, color, size = 18) {
  if (icon) {
    return `<img
      src="https://cdn.jsdelivr.net/gh/atomiclabs/cryptocurrency-icons/svg/color/${icon}.svg"
      width="${size}" height="${size}"
      style="display:block;border-radius:50%"
      onerror="this.outerHTML='<span style=font-size:${Math.floor(size*0.5)}px>${init}</span>'">`;
  }

  return `<span style="font-size:${Math.floor(size*0.5)}px;font-weight:700">${init}</span>`;
};
...
function initTVChart(symbol, interval) {
  ...
  script.innerHTML = JSON.stringify({
    ...
    overrides: {
      ...
      "volume.volume.color.0": "rgba(255,71,87,0.3)",
      "volume.volume.color.1": "rgba(0,194,122,0.3)"
    },
    ...
    return `
      <div class="ob-batch-block ${isLatest ? 'ob-batch-latest' : ''}">
        ...
        <div class="ob-batch-mid">
          <span class="ob-mid-price-val">${batch.mid.toLocaleString('en-US', {maximumFractionDigits:6})}</span>
          <span class="ob-spread-small">Spread ${batch.spread}</span>
        </div>
...
const side       = tradeData.side || 'buy';
  ...
  try {
    // Step 1: Execute on-chain via Aftermath Router
    let txDigest = null;
  ...
  try {  
    ...
    window.archiveTrade(tradeData).then(result => {
      if (EXEC_LOG[0] && result?.blobId) {
        EXEC_LOG[0].blobId      = result.blobId;
        EXEC_LOG[0].retrieveUrl = result.retrieveUrl;
    EXEC_LOG[0].txUrl = result.txUrl
  || (EXEC_LOG[0].txDigest ? `https://suiscan.xyz/mainnet/tx/${EXEC_LOG[0].txDigest}` : null);
        EXEC_LOG[0].network     = result.network;
        _saveExecLog(EXEC_LOG);
        renderExecFeed();
        _syncSwapHistory();
      }
    }).catch(archiveErr => {
      console.error('[Gambit] Walrus archive failed (non-fatal):', archiveErr);
    });
...
function _syncSwapHistory() {
  ...
  body.innerHTML = EXEC_LOG.map(r => `
    <div class="acct-history-row">
      <span class="acct-h-pair">${r.pair}</span>
...
function renderExecFeed() {
  ...
  body.innerHTML = EXEC_LOG.map((r, i) => {
    // Build archive link if we have a real blobId
    const archiveLink = r.blobId
      ? `<a class="exec-blob-link" href="${r.retrieveUrl || '#'}" target="_blank" rel="noopener"
           title="View blob on Walrus aggregator">
           ◈ ${r.blobId.slice(0, 8)}…
         </a>`
      : '<span class="exec-blob-pending">—</span>';

    // Build tx link if we have a real digest
    const txLink = r.txDigest && r.txUrl
      ? `<a class="exec-tx-link" href="${r.txUrl}" target="_blank" rel="noopener"
    ...
    return `
      <div class="exec-row ${i === 0 ? 'exec-row-new' : ''}">
        ...
        <span class="exec-pair">${r.pair}</span>
        ...
        <span class="exec-archive">${archiveLink}</span>
...
function setSide(side, el) {
  ...
  if (STATE.connected) {
    const label = side === 'buy'
      ? (STATE.usdcBalance || 0).toFixed(2) + ' USDC'
...
STATE.side = side;
  ...
  const quote  = (STATE.marketSym || 'SUI/USDC').split('/')[1].trim();
...
['exec-btn', 'exec-limit-btn', 'exec-twap-btn'].forEach(id => {
  ...
  if (!STATE.connected && balEl) {
    balEl.textContent = side === 'buy' ? '0.00 USDC' : '0.00 ' + ticker;
  }
...
window.onStripTick = function(data) {
  ...
  if (volEl && data.vol24h != null)
    volEl.textContent = typeof data.vol24h === 'number' ? fmt(data.vol24h) + ' USDC' : data.vol24h;
...
if (sizeUnitElM) sizeUnitElM.textContent = tokenBase;
  ...
  const _execUnit = _execSide === 'buy' ? tokenQuote : tokenBase;
  ['exec-size-unit','exec-limit-size-unit','exec-twap-size-unit'].forEach(id => {
    const u = document.getElementById(id);
  ...
  if (balanceEl)  balanceEl.textContent  = '0.00 ' + tokenQuote;
...
// Strip stats — cleared on market switch, real values arrive via onStripTick()
  ...
  if (m.price) {
    ...
    if (volEl && m.vol) volEl.textContent = m.vol >= 1e9 ? (m.vol/1e9).toFixed(2)+'B USDC'
                                          : m.vol >= 1e6 ? (m.vol/1e6).toFixed(2)+'M USDC'
...
function _updateAccountPage() {
  const addrEl = document.getElementById('acct-wallet-addr');
  if (!STATE.connected) return;
  const short = STATE.walletAddr.slice(0,6) + '…' + STATE.walletAddr.slice(-4);
  if (addrEl) {
    addrEl.textContent = short;
    addrEl.style.fontFamily = 'var(--mono)';
    addrEl.style.color = 'var(--gold)';
  }

  // Switch empty state to show connected message
  const balPanel = document.getElementById('acct-balances');
  if (balPanel) {
    const sui  = (STATE.suiBalance  || 0).toFixed(4);
    const usdc = (STATE.usdcBalance || 0).toFixed(2);
    balPanel.innerHTML = `
      <div class="acct-token-row">
        ...
        <div class="acct-token-right">
          <span class="acct-token-amount">${sui}</span>
          <span class="acct-token-usd">SUI</span>
        </div>
      </div>
      <div class="acct-token-row">
        <div class="acct-token-left">
          <span class="acct-token-badge usdc">USDC</span>
          <span class="acct-token-name">USD Coin</span>
        </div>
        <div class="acct-token-right">
          <span class="acct-token-amount">${usdc}</span>
          <span class="acct-token-usd">USDC</span>
        </div>
      </div>
    `;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  navigate('markets');
  openMarket('SUI/USDC');
  STATE.marketSym = 'SUI/USDC';  // ensure STATE is in sync before any updateCTA calls    
...
if (mkt?.price) STATE.midPrice = mkt.price;
  ...
  function runBatch() {
    ...
    // Fetch real on-chain orderbook from Aftermath pool
