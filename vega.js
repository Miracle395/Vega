...
const stubRoutes = ['vaults','staking','referrals','leaderboard'];
...
// Archive
  ...
  if (page === 'earn') { fetchPoolMetrics(); if (window.STATE?.walletAddr) fetchUserBalances(window.STATE.walletAddr); }
...
function _showTradeSuccessCard({ side, baseToken, quoteToken, amountIn, amountOut, txDigest, route }) {
  // Remove any existing card
  const existing = document.getElementById('vega-success-card');
  if (existing) existing.remove();

  const card = document.createElement('div');
  card.id = 'vega-success-card';
  card.innerHTML = `
    <div style="
      position:fixed;bottom:80px;left:50%;transform:translateX(-50%);
      width:calc(100% - 32px);max-width:400px;
      background:#0a0a0a;color:#d8f8d8;
      border-radius:16px;padding:20px;
      box-shadow:0 8px 32px rgba(0,0,0,0.4);
      z-index:9999;font-family:'Geist',sans-serif;
      animation:slideUp .25s ease;
    ">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:28px;height:28px;border-radius:50%;background:#d8f8d8;display:flex;align-items:center;justify-content:center;">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0a0a0a" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
          </div>
          <span style="font-weight:700;font-size:14px;">Trade Confirmed</span>
        </div>
        <button onclick="document.getElementById('vega-success-card').remove()" style="background:none;border:none;color:rgba(216,248,216,0.5);cursor:pointer;font-size:18px;line-height:1;">×</button>
      </div>
      <div style="background:rgba(216,248,216,0.08);border-radius:10px;padding:12px;margin-bottom:14px;">
        <div style="font-size:11px;color:rgba(216,248,216,0.5);margin-bottom:4px;text-transform:uppercase;letter-spacing:.06em;">${side === 'buy' ? 'Bought' : 'Sold'}</div>
        <div style="font-size:22px;font-weight:700;font-family:'Geist Mono',monospace;letter-spacing:-.02em;">
          ${side === 'buy' ? parseFloat(amountOut).toFixed(4) + ' ' + baseToken : parseFloat(amountIn).toFixed(4) + ' ' + baseToken}
        </div>
        <div style="font-size:11px;color:rgba(216,248,216,0.5);margin-top:4px;">
          via ${route}
        </div>
      </div>
      <div style="display:flex;gap:8px;">
        <a href="https://suiscan.xyz/testnet/tx/${txDigest}" target="_blank" style="
          flex:1;padding:10px;border-radius:10px;
          border:1px solid rgba(216,248,216,0.2);
          color:#d8f8d8;font-size:11px;font-weight:600;
          text-align:center;text-decoration:none;
          letter-spacing:.04em;text-transform:uppercase;
        ">View on Suiscan</a>
        <button onclick="_shareTradeCard('${side}','${amountIn}','${amountOut}','${baseToken}','${quoteToken}','${txDigest}')" style="
          flex:1;padding:10px;border-radius:10px;
          background:#d8f8d8;color:#0a0a0a;
  ...
  setTimeout(() => { if (document.getElementById('vega-success-card')) card.remove(); }, 8000);
}

window._shareTradeCard = function(side, amountIn, amountOut, baseToken, quoteToken, txDigest) {
  const text = side === 'buy'
    ? `Just bought ${parseFloat(amountOut).toFixed(4)} ${baseToken} on Vega \nPowered by DeepBook V3 on Sui\n\nvegaxyz.vercel.app`
    : `Just sold ${parseFloat(amountIn).toFixed(4)} ${baseToken} on Vega \nPowered by DeepBook V3 on Sui\n\nvegaxyz.vercel.app`;
  if (navigator.share) {
...
window.executeTrade = async function() {
  ...
  try {
   ...
   // Reset form fully
    ...
    const sizeInputM = document.getElementById('trade-size-input-m');
    if (sizeInputM) { sizeInputM.value = ''; sizeInputM.disabled = false; }
    const slider = document.getElementById('trade-slider');
    const sliderM = document.getElementById('trade-slider-m');
    const pct    = document.getElementById('trade-pct');
    const pctM   = document.getElementById('trade-pct-m');
    if (slider) slider.value = 0;
    if (sliderM) sliderM.value = 0;
    if (pct)    pct.textContent = '0%';
    if (pctM)   pctM.textContent = '0%';
    updateCTA();
    
   // Refresh balances immediately after swap
    if (typeof window._refreshWalletBalances === 'function') {
      window._refreshWalletBalances();
    } else if (window._fetchWalletBalances && window.STATE?.walletAddr) {
      window._fetchWalletBalances(window.STATE.walletAddr);
    }

  } catch (err) {
    console.error('[Vega] Trade execution error:', err);
    if (btn) {
