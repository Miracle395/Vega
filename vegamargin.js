...
async function fetchPoolMetrics() {
  try {
    const [suiPool, dbPool] = await Promise.all([
      fetch(`${MARGIN.INDEXER}/margin_pool_info?pool_id=${MARGIN.SUI_POOL}`).then(r => r.json()),
      fetch(`${MARGIN.INDEXER}/margin_pool_info?pool_id=${MARGIN.DBUSDC_POOL}`).then(r => r.json()),
    ]);

    MG.poolMetrics.sui   = Array.isArray(suiPool)   ? suiPool[0]   : suiPool;
    MG.poolMetrics.dbusdc = Array.isArray(dbPool)   ? dbPool[0]   : dbPool;

    renderPoolMetrics();
  } catch (err) {
    console.warn('[Margin] Indexer fetch failed:', err.message);
    // Try alternative endpoint format
    fetchPoolMetricsFallback();
  }
}

async function fetchPoolMetricsFallback() {
  try {
    const res = await fetch(`${MARGIN.INDEXER}/margin_pools`);
    const data = await res.json();
    if (Array.isArray(data)) {
      MG.poolMetrics.sui    = data.find(p => p.pool_id === MARGIN.SUI_POOL);
      MG.poolMetrics.dbusdc = data.find(p => p.pool_id === MARGIN.DBUSDC_POOL);
      renderPoolMetrics();
    }
  } catch (err) {
    console.warn('[Margin] Fallback fetch failed:', err.message);
  }
}
...
function renderPoolMetrics() {
  ...
  if (sui) {
    const totalSupply  = Number(sui.total_supply  || sui.totalSupply  || 0);
    const totalBorrows = Number(sui.total_borrows || sui.totalBorrows || 0);
    const util = totalSupply > 0 ? totalBorrows / totalSupply : 0;
    const apr  = sui.borrow_apr || sui.borrowApr || sui.interest_rate || null;
    const liq  = totalSupply - totalBorrows;

    setEl('sui-liquidity',    fmtAmt(liq, MARGIN.SUI_SCALAR) + ' SUI');
    setEl('sui-borrow-apr',   apr ? fmtPct(apr) : '—');
    setEl('sui-borrow-apr-2', apr ? fmtPct(apr) : '—');
    setEl('sui-deposit-apr',  apr ? fmtPct(Number(apr) * 0.8) : '—');
    ...
    setEl('mg-active-loans', fmtAmt(totalBorrows, MARGIN.SUI_SCALAR) + ' SUI');
  }

  if (db) {
    const totalSupply  = Number(db.total_supply  || db.totalSupply  || 0);
    const totalBorrows = Number(db.total_borrows || db.totalBorrows || 0);
    const apr = db.borrow_apr || db.borrowApr || db.interest_rate || null;

    setEl('dbusdc-total-supply', fmtAmt(totalSupply, MARGIN.DBUSDC_SCALAR) + ' DBUSDC');
    setEl('dbusdc-supply-apr',   apr ? fmtPct(Number(apr) * 0.8) : '—');
    setEl('dbusdc-apy',          apr ? fmtPct(Number(apr) * 0.8) : '—');
    setEl('dbusdc-earn-rate',    apr ? fmtPct(Number(apr) * 0.8) : '—');
    setEl('mg-total-deposits',   fmtAmt(totalSupply, MARGIN.DBUSDC_SCALAR) + ' DBUSDC');
  }
}
...
window.handleConnect = function(btn) {
  window._connectSuiWallet(btn);
};

// After wallet connects, fetch balances + risk ratio
const _origFetch = window._fetchWalletBalances;
window._fetchWalletBalances = async function(addr) {
  if (_origFetch) await _origFetch(addr);
  fetchUserBalances(addr);

  // Sync connect button in margin nav
  ...
  // Check for existing margin manager
  try {
    const res  = await fetch(`${MARGIN.INDEXER}/margin_managers_info?owner=${addr}`);
    const data = await res.json();
    if (Array.isArray(data) && data.length > 0) {
      MG.marginManagerId = data[0].margin_manager_id;
      updateRiskRatio(MG.marginManagerId);
    }
  } catch (_) {}
};
